...
export function MarketValue({car,pool,expanded=false,onOpen}:{car:Listing;pool:Listing[];expanded?:boolean;onOpen?:(car:Listing)=>void}){
 ...
 return <section className={`market-value market-${value.rating} ${expanded?'':'market-compact'}`} aria-label={`MotorScout price check: ${label}`}>
  ...
  <small className="market-disclosure">{value.regional?`${car.state} listings`:'Nationwide comparison'} · Asking prices, not sale values.</small>
  {expanded&&<><details className="market-method"><summary>How we compare prices</summary><p>The middle 50% of asking prices for unique cars in your current search: same make, model, trim and seller type, within one model year and 20,000 miles. We prefer your car’s state when at least five matches exist. Unusual prices and listings older than 30 days are excluded.</p><p>Your search filters can narrow or bias this sample. We do not adjust for unknown condition, accident history, equipment, regional differences, taxes or fees. A green rating means a lower asking price, not a verified good deal. This is an asking-price comparison, not a professional appraisal or trade-in valuation.</p></details><h3>Similar cars used for this estimate</h3><div className="market-comps">{value.comparables.map(c=><button key={c.id} type="button" onClick={()=>onOpen?.(c)} disabled={!onOpen}><span>{c.title}<small>{c.miles?.toLocaleString()} mi · {[c.city,c.state].filter(Boolean).join(', ')}</small></span><strong>{money(c.price)}</strong></button>)}</div></>}
 </section>;
}
