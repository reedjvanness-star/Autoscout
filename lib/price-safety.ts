...
export const DISCOUNT_PRICE_WARNING='This dealer format can report savings or accessory charges instead of the vehicle price. The full asking price must be verified before this amount can be displayed or used for your budget.';
...
export function discountRisk(row:Pick<Listing,'url'|'price'|'year'|'miles'>){
 ...
 return url.hostname.replace(/^www\./,'')==='vandergriffacura.com'||/^\/(?:used|certified|new)-[^/]+-[A-HJ-NPR-Z0-9]{17}\/?$/i.test(path)||
