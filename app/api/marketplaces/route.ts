...
export async function POST(req:Request){try{
 ...
 const hasMore=(j:Job)=>!facebook&&marketplaceRegionBatch('automotive',w.filters.state,j.batch??0).hasMore;
 if(a.action==='start'){
  ...
  try{
   const data=await startBudgetedMarketplaceRun(db(),key,actor,facebook?facebookInput(w.filters):marketplaceInput(w.filters,next.batch),facebook?1:MARKETPLACE_RUN_CAP);
 ...
 const sources:Source[]=facebook?[{name:'Facebook Marketplace',status:rows.length?'searched':done?'error':'ready',count:rows.length,inspected:rows.length,detail:`Local search centers: ${marketplaceRegionBatch('facebook',latest.filters.state).regions.join(', ')}. Partial coverage only. ${rows.length} usable vehicles returned. Your exact filters are applied before display.`}]:marketplaceSources(rows,done,latest.filters.state,job.batch??0);latest.sources=[...latest.sources.filter(s=>!sources.some(n=>n.name===s.name)),...sources];
